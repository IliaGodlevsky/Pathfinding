...
using Command = Pathfinding.Service.Interface.IPathfindingRangeCommand<Pathfinding.Presentation.Console.Models.GraphVertexModel>;
...
internal sealed class RunRangeViewModelTests
{
    ...
    public async Task AddToRangeCommand_ShouldExecuteFirstAvailableCommand()
    {
        ...
        rangeServiceMock
            .Setup(x => x.ReadRangeOrderedAsync(
                It.IsAny<int>(),
    ...
    public async Task DeletePathfindingRange_ShouldClearStoredRangeAsync()
    {
        ...
        rangeServiceMock
            ...
            .ReturnsAsync(
            [
                new() { VertexId = 1, Order = 0 },
                new() { VertexId = 2, Order = 1 }
            ]);
