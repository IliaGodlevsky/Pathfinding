...
internal sealed class ServiceIntegrationTests
{
    ...
    public async Task CreatePathfindingHistory_WithRangeAndRuns_ShouldRoundTripCriticalData()
    {
        ...
        [
            ...
            {
                GraphId = graph.Id,
                Algorithm = Domain.Enums.Algorithms.AStar,
    ...
    public async Task DeleteGraph_WithRelatedRunsAndRange_ShouldCascadeDeleteAllDependentData()
    {
        ...
        [
            ...
            {
                GraphId = graph.Id,
                Algorithm = Domain.Enums.Algorithms.Dijkstra,
                Visited = 2,
                Steps = 1,
                Cost = 1,
        ...
        var rangeAfterDelete = await rangeService.ReadRangeOrderedAsync(graph.Id);
        var runsAfterDelete = await statisticsService.ReadStatisticsAsync(new ReadStatisticsRequest(graph.Id));
    ...
    public async Task DeleteRunsAsync_ShouldDeleteOnlyRequestedRuns_AndKeepRestReadable()
    {
        ...
        var afterDelete = await statisticsService.ReadStatisticsAsync(new ReadStatisticsRequest(graph.Id));
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(afterDelete.Single().Algorithm, Is.EqualTo(Domain.Enums.Algorithms.Dijkstra));
        }
    }
