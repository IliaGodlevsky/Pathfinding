...
internal class StatisticsRequestServiceTests
{
    ...
    public async Task ReadStatisticsByGraphAsync_ShouldReturnModels()
    {
        ...
        {
            new() { Id = 3, GraphId = 4, Algorithm = Domain.Enums.Algorithms.AStar, ResultStatus = RunStatuses.Success, Steps = 2, Visited = 2, Cost = 2, Elapsed = 2 }
        };
        repository
            .Setup(x => x.ReadByGraphIdAsync(4, 0, int.MaxValue))
        ...
        var result = await service.ReadStatisticsAsync(new ReadStatisticsRequest(4));
