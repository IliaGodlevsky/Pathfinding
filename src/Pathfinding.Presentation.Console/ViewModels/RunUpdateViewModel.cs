...
internal sealed class RunUpdateViewModel : ViewModel, IRunUpdateViewModel, IDisposable
{
    ...
    private async Task<(Graph<GraphVertexModel> Graph, int GraphId)> EnsureGraphLoadedAsync(AwaitGraphStructureUpdatedMessage msg)
    {
        ...
        await ExecuteSafe(async token =>
        {
            ...
            graphId = model.Id;
            var layer = neighborFactory.Create(model.Neighborhood);
            await layer
    ...
    private async Task NotifyRunsUpdatedAsync(int graphId, Graph<GraphVertexModel> graphToUpdate)
    {
        ...
        {
            var models = await statisticsService
                .ReadStatisticsAsync(new ReadStatisticsRequest(graphId), token)
                .ConfigureAwait(false);
        ...
        ActiveGraph graphToUpdate)
    {
        var range = (await rangeService
            .ReadRangeOrderedAsync(graphToUpdate.Id)
            .ConfigureAwait(false))
    ...
    private RunStatisticsModel UpdateStatistics(RunStatisticsModel info, GraphVertexModel[] range)
    {
        ...
        var factory = algorithmsFactory.Create(info.Algorithm);
        var algorithm = factory.Create(range, info);
