...
public class PathfindingHistorySerializationModel : IBinarySerializable, IXmlSerializable
{
    ...
    public IReadOnlyCollection<CoordinateSerializationModel> Range { get; set; }
    ...
    public async Task DeserializeAsync(Stream stream, CancellationToken token = default)
    {
        ...
        Range = await stream.ReadSerializableArrayAsync<CoordinateSerializationModel>(token).ConfigureAwait(false);
    }
    ...
    public void ReadXml(XmlReader reader)
    {
        ...
        Range = reader.ReadCollection<CoordinateSerializationModel>(nameof(Range), "Coordinates");
    }
